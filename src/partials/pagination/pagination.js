...
export function createPaginationSearch(totalPages, page) {
  let liTag = '';
  let active;
  let currPage = page;
  let beforePage = page - 1;
  ...
  if (page > 1) {
    //show the next button if the page value is greater than 1
    liTag += `<li class="btn prev" onclick="getMoviesSearch(${totalPages}, ${
      page - 1
    })"><span> << </span></li>`;
  }
  ...
  if (page > 2) {
    //if page value is less than 2 then add 1 after the previous button
    liTag += `<li class="first numb" onclick="getMoviesSearch(${totalPages}, 1)"><span>1</span></li>`;
    if (page > 3) {
      //if page value is greater than 3 then add this (...) after the first li or page
      liTag += `<li class="dots"><span>...</span></li>`;
    }
  ...
  for (var plength = beforePage; plength <= afterPage; plength++) {
    ...
    liTag += `<li class="numb ${active}" onclick="getMoviesSearch(${totalPages}, ${plength})"><span>${plength}</span></li>`;
  }
  ...
  if (page < totalPages - 1) {
    ...
    liTag += `<li class="last numb" onclick="getMoviesSearch(${totalPages}, ${totalPages})"><span>${totalPages}</span></li>`;
  }
  ...
  if (page < totalPages) {
    ...
    liTag += `<li class="btn next" onclick="getMoviesSearch(${page}, ${
      page + 1
    })"><span> >> </span></li>`;
  }
